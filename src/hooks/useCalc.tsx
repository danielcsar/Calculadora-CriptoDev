...
const useCalc = () => {
  ...
  const operar = (oper: string) => {
    ...
    try {
      const conta = (valorTela: string) => {
        return (new Function( 'return ' + valorTela)());
      }
      setResultado(conta(valorTela));
      setOperacao(true);
    } catch (error) {
      setResultado("ERROR");
    }
  ...
  useEventListener("keydown", (e: KeyboardEvent) => {
    if (
      ...
      e.key === "Enter") 
      {
        if (e.key === "=" || e.code.startsWith("Enter") || e.code.startsWith("NumpadEnter")) {          
        operar("=");
        return;
        }
